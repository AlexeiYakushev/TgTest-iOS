...
public class ItemListCallListItem: ListViewItem, ItemListItem {
    ...
    public init(presentationData: ItemListPresentationData, dateTimeFormat: PresentationDateTimeFormat, messages: [Message], sectionId: ItemListSectionId, style: ItemListStyle, displayDecorations: Bool = true, serverDate: Int? = nil) {
...
public class ItemListCallListItemNode: ListViewItemNode {
    ...
    public func asyncLayout() -> (_ item: ItemListCallListItem, _ params: ListViewItemLayoutParams, _ insets: ItemListNeighbors) -> (ListViewItemNodeLayout, () -> Void) {
        ...
        return { [weak self] item, params, neighbors in
			...
			let date = item.serverDate != nil ? Int32(item.serverDate!) : earliestMessage.timestamp
            let titleText = stringForDate(timestamp: date, strings: item.presentationData.strings)
